...
class RandomSampledDataset():
    def __init__(self, datasets_root, dataset_name):
        ...
        self._ids = [file_name.split('.png')[0] for file_name in os.listdir(os.path.join(self.dataset_path, 'img'))]
    ...
    def fetch_img(self, image_id):
        img_path = os.path.join(self.dataset_path, 'img', '{}.png'.format(image_id))
        raw_image = Image.open(img_path).convert('RGB')
